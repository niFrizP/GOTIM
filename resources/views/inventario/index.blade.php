                        ...
                        @foreach ($inventarios as $inv)
                            <tr>
                                ...
                                <td class="border-b p-2">
                                    <span
                                        class="inline-block rounded px-2 py-1 text-xs font-semibold {{ $inv->estado === 'activo' ? 'bg-green-200 text-green-800 dark:bg-green-900/20 dark:text-green-300' : 'bg-red-200 text-red-800 dark:bg-red-900/20 dark:text-red-300' }}">
                                        {{ ucfirst($inv->estado) }}
                                    </span>
                                </td>
                                <td class="border-b p-2 space-x-2">
                                    <a href="{{ route('inventario.ver', $inv->id_inventario) }}"
                                    ...
                                    @if (Auth::user()->rol === 'Administrador')
                                        ...
                                        @else
                                            <a href="{{ route('inventario.reactivar', $inv->id_inventario) }}"
                                                class="text-green-500 hover:underline">Reactivar</a>
                                        @endif
                                    @endif
                                </td>
                            </tr>
