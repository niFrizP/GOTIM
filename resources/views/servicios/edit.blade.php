        ...
        <div class="mx-auto max-w-4xl sm:px-6 lg:px-8">
            <div class="rounded bg-white dark:bg-gray-800 p-6 shadow">
                <form method="POST" action="{{ route('servicios.update', $servicio->id_servicio) }}">
                    ...
                    <div class="mt-6">
                        <x-primary-button type="submit" id="btnActualizar">Actualizar Servicio</x-primary-button>
            ...
            const validarNombre = async (nombre) => {
                try {
                    ...
                    if (data.disponible) {
                        ...
                        btnActualizar.disabled = false; // Habilitar botón si el nombre es válido
                    } else {
                        msg.textContent = '❌ Nombre ya está en uso';
                        msg.className = 'mt-1 text-sm text-red-600';
                        btnActualizar.disabled = true; // Deshabilitar botón si el nombre no es válido
                    }
                } catch (error) {
                    msg.textContent = '⚠️ Error al validar';
                    msg.className = 'mt-1 text-sm text-yellow-600';
                    btnActualizar.disabled = true; // Deshabilitar botón en caso de error
                }
            };
